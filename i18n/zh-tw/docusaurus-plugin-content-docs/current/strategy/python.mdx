...
系統呼叫 `strategy.trade()` 時，會傳入 `information` 物件，以提供以下資訊： `candles` (K 線資訊), `orders` (訂單）, `availableAssets` (可用資產）, `totalAssets` (所有資產）．取用方法為:

```python
information['candles']
information['history_data']
information['orders']
information['availableAssets']
information['totalAssets']
```
...
策略擁有隨時更新的 `availableAssets`, `totalAssets` 資產資訊，即當前可進行操作的貨幣，取用方式如下:
...
回傳對應的交易紀錄，使用 `dictionary` 紀錄成交訂單資訊
...
取回最後 3 筆成交紀錄，並印出價量資訊。

```python
orders = CA.get_orders('FILLED', 3)
CA.log('len ' + str(len(orders)))
for order in orders:
    CA.log( 'amount: ' + str(order['amount']) + ' ,price: ' + str(order['price']))
```
